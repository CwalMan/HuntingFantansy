...
public class InventoryManager : MonoBehaviour
{
    ...
    */
    ...
    void SwapItem(BaseItemSlot _itemSlot)
    {
        ...
        {
            if (dragWeaponItem != null) weaponPanel.Equip(dragWeaponItem);  // 아이템 없으면
            if (dropWeaponItem != null) weaponPanel.Equip(dragWeaponItem); // 아이템 있으면
        }
        if (dragItemSlot is WeaponSlot) // 드래그한 곳이 무기슬롯 일 때
        {
            if (dragWeaponItem != null) weaponPanel.UnEquip(dragWeaponItem);
            if (dropWeaponItem != null) weaponPanel.Equip(dropWeaponItem);
        }
    ...
    public void UnequipWeapon(WeaponSO item)
    {
        if(inventory.CanAddItem(item))
        {
            ...
            {
                item.Unequip(this); // Remove Weapon item Stat
                weaponPanel.UnEquip(item);
                inventory.AddItem(item);
            }
        }
    }
